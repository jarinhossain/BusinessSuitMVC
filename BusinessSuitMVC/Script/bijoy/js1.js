...
function ChangeConverterStatus() {
    ...
    { field[0].checked = true; }
        ...
        if (ConvertFrom == "somewherein")
        { field[1].checked = true; }

        else
            if (ConvertFrom == "boisakhi")
...
function KeyboardLayoutOptionClick(event) {
    ...
    for (var counter = 0; counter < field.length; counter++) {
        if (field[counter].checked) {
            KeyBoardLayout = counter + 1; ChangeKeyboarLayoutStatus();
...
function ConvertFromTextArea(idcvt) {
    var str = document.getElementById(idcvt).value; var splitted = str.split(/([\u0980-\u09FF]+\s*)/g);
    str = ""; for (var i = 0; i < splitted.length; i = i + 2) {
        if (splitted[i] == " ")
        { str += " "; }

        else
        {
            str += ConvertToUnicode(ConvertFrom, splitted[i]);
        }

        str += (splitted[i + 1] === undefined) ? '' : splitted[i + 1];
    }
...
function ConvertToTextArea(val) {
    var str = val; str = ConvertToASCII(ConvertFrom, str);
    return str;
...
function ConvertOptionChange(event) {
    ...
    for (var counter = 0; counter < field.length; counter++) {
        if (field[counter].checked) {
            if (counter == 0)
                ConvertFrom = "bijoy"; else
                if (counter == 1)
                    ConvertFrom = "somewherein"; else
                    if (counter == 2)
...
function OnPageLoad() {
    ...
    myFld.style.width = 400; var convertarea = document.getElementById('CONVERTEDT');
